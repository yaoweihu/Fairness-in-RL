...
from allocation.config import SAVE_DIR
...
class CPO:
    @autoassign
    def __init__(self, policy, value_fun, cost_fun, simulator, max_kl=1e-2, max_val_step=1e-3,
                 max_cost_step=1e-3, max_constraint_val=0.1, val_iters=1, cost_iters=1, val_l2_reg=1e-3,
                 ...
                 line_search_max_iter=10, line_search_accept_ratio=0.1, model_name='cpo_agent', save_path=None,
                 model_path=None, continue_from_file=False, save_every=25, print_updates=True):
        ...
        if not save_path and save_every:
...
#             constraint_cost = torch.mean(torch.tensor([disc_costs[start] for start in trajectory_limits[:-1]]))
    ...
    def save_session(self):
        ...
        f_name = self.model_name + f'_ep{self.episode_num}.pt'
        save_path = os.path.join(self.save_path, f_name)
