...
from infectious.agents.ppo.sb3.type_aliases import (
    ReplayBufferSamples, 
...
class RolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.long_deltas = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.generator_ready = False
    ...
    def add(
        ...
        self.long_deltas[self.pos] = long_deltas.clone().cpu().numpy()
        self.pos += 1
        if self.pos == self.buffer_size:
            self.full = True
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> RolloutBufferSamples:
        data = (
            ...
            self.long_deltas[batch_inds].flatten(),
        )
