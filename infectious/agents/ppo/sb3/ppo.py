...
from infectious.config import REGULARIZE_ADVANTAGE, BETA_0, BETA_1, BETA_2, BETA_3, BETA_4, OMEGA
from infectious.agents.ppo.sb3.on_policy_algorithm import OnPolicyAlgorithm
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                if REGULARIZE_ADVANTAGE: 
                    ...
                    long_term = torch.min(torch.zeros(rollout_data.long_deltas.shape[0]).cuda(),
                                         -div_cond * rollout_data.long_deltas)
                    long_term1 = torch.max(torch.zeros(rollout_data.long_deltas.shape[0]).cuda(),
                                         -(1. - div_cond) * rollout_data.long_deltas)
                    ...
                    long_term = (long_term - torch.min(long_term)) / (torch.max(long_term) - torch.min(long_term) + 1e-8)
                    long_term1 = (long_term1 - torch.min(long_term1)) / (torch.max(long_term1) - torch.min(long_term1) + 1e-8)

                    # Add terms to advantages
                    advantages = (BETA_0 * advantages + BETA_1 * vt_term + BETA_2 * div_term + BETA_3 * long_term + BETA_4 * long_term1)
